...
export async function upsertSessionIndex(meta) {
  const current = await getJSON(keys.sessionIndex(), []);
  const arr = Array.isArray(current) ? current : [];
  const nextMeta = {
    sessionId: meta.sessionId,
    title: meta.title || "新しいセッション",
    createdAt: meta.createdAt || Date.now(),
    updatedAt: meta.updatedAt || Date.now(),
    messagesCount: meta.messagesCount || 0,
    lastPreview: meta.lastPreview || ""
  };
  const next = [nextMeta, ...arr.filter((x) => x?.sessionId !== nextMeta.sessionId)]
    .sort((a, b) => (b.updatedAt || 0) - (a.updatedAt || 0))
    .slice(0, 80);
  await setJSON(keys.sessionIndex(), next);
  await setJSON(keys.sessionMeta(nextMeta.sessionId), nextMeta);
  return next;
}
...
export const keys = {
  ...
  sessionIndex: () => "sessions:index:hisa",
